...
A well-trained CNN model should detect image regions that best discriminate classes as positive activations, improve class separation in those regions by eliminating negative activations, and aggregate the resulting activations (pooling and flattening) into a highly dimensional and sparse feature space, in which even a linear classifier should separate the classes with reasonable success. Nevertheless, it is common to assume that the classes are at least piecewise linearly separable and use fully connected layers -- a Multi-Layer Perceptron (MLP) classifier -- for feature space reduction and classification. The training of a CNN usually rely on  weight optimization by backpropagation, which requires a higher number of annotated images as deeper is the model. In FLIM, we eliminate the need for backpropagation by finding a set ${\cal F}$ of filters that can detect discriminant regions from all classes. 

In FLIM, the user selects a few training images $I$ to compose a very small dataset $\D$ and draws labeled markers in image regions that best discriminate the classes. We wish to estimate the weights of filters $F\in {\cal F}$, such that the convolution between an image $I$ and $F$ will generate positive activations in regions whose local patterns are detected by $F$. In order to understand this process, one must interpret the convolution operation at a pixel $p$ of $I$ as the inner product $\vect\left[P_I(p)\right] \cdot  \vect\left[F\right]$, where $P_I(p)$ is a patch with $k\times k$ pixels and $m$ image channels around $p$, $F$ is a filter with $k\times k\times m$ weights, and $\vect\left[.\right]$ is the vectorization operation. A filter $F$ can discriminate a class $i\in \{1,2,\ldots,c\}$ among $c$ classes when it generates positive results for local patterns $\vect\left[P_I(p)\right]$ of class $i$ and negative results for patterns from other classes. That is, $\vect\left[F\right]$ plays the role of a normal vector of a hyperplane in $\Re^{k\times k\times m}$ and we expect that ${\cal F}$ will contain filters to detect local patterns from all classes in different positions $p$. 
...
\label{eq.pset}
\end{eqnarray}
The patch set ${\cal P}$ defines samples from all classes in $\Re^{k\times k\times m}$ and we should expect that a clustering operation will create groups with similar local patterns. Ideally, each group should contain local patterns of a same class. However, especially in the first layers, some classes might share similar patterns and we should at least expect that they will be detected in different regions of the image. The groups must be shifted to the origin of $\Re^{k\times k\times m}$ by subtracting the mean vector of the samples in ${\cal P}$. Optionally, the standard deviation of each feature can be computed and used for feature standardization. We call this operation \emph{patch-based normalization}. The centroid of each cluster defines the weights of each filter $F\in {\cal F}$ and we also force norm $1$ to avoid preferences among them. This centralization for filter definition is paramount to eliminate activations in regions from other classes, whose patterns fall in the negative side of the hyperplane represented by $\vect\left[F\right]$. Figure~\ref{fig:filter} illustrates how it works. ....
