...
\begin{figure*}
  \begin{center}
    ...
    \tikzstyle{steps} = [rectangle, rounded corners, minimum width=1.3cm, minimum height=0.7cm,text centered, draw=black,fill=violet!30, execute at begin node={\begin{varwidth}{2.5cm}},
    ...
    \begin{tikzpicture}[node distance=1.5cm]
     ...
     \node (imgs) [fit={($(img1.north west)+(-0.45,0.2)$) ($(img3.south east)+(0.45,-0.2)$)}, inner sep=0pt, draw=none, rounded corners, very thick, label = {[label distance = 0.3em]below:{Images}}] {};
     %----------------------------------

     \node (tsne) [steps, right=1em of imgs] {t-SNE};

     \node (vis1) [image, right=1.5em of tsne, outer sep=-15pt] {\includegraphics[width=.15\textwidth]{figures/input.png}};

     \node (select) [steps, right=1.35em of vis1] {Select Images};
    
    %images----------------------------
    \node (simg1) [image, above right=-2em and 3em of select] {\includegraphics[width=.08\textwidth]{figures/img1.png}};
    ...
    \node (simgs) [fit={($(simg1.north west)+(-0.64,0.2)$) ($(simg4.south east)+(0.64,-0.2)$)}, inner sep=0pt, draw=none] {};
    %----------------------------------

     \node (put) [steps, right=1em of simgs] {Put Markers};

     %images----------------------------
    \node (mimg1) [image, above right=-2em and 3em of put] {\includegraphics[width=.08\textwidth]{figures/markers1.png}};
    ...
    \node (mimgs) [fit={($(mimg1.north west)+(-0.64,0.2)$) ($(mimg4.south east)+(0.64,-0.2)$)}, inner sep=0pt, draw=none] {};
    %----------------------------------

     \node (flim) [steps, below left=5em and -3em of mimgs] {FLIM};

     \node (ft) [steps, left=2em of flim] {Feature Extractor};

     \node (aft) [steps, left=2em of ft] {Apply Featute \\ Extractor};

      %images----------------------------
    \node (f1) [image, above left=-2.8em and 2em of aft] {\includegraphics[width=.07\textwidth]{figures/features/f1.png}};
    \node (f2) [image, below left=-5em of f1] {\includegraphics[width=.07\textwidth]{figures/features/f2.png}};
    \node (f3) [image, below left=-5em of f2] {\includegraphics[width=.07\textwidth]{figures/features/f3.png}};
    \node (f4) [image, below left=-5em of f3] {\includegraphics[width=.07\textwidth]{figures/features/f4.png}};

    \node (features) [fit={($(f1.north west)+(-0.17,0.2)$) ($(f4.south east)+(0.17,-0.2)$)}, draw=none, inner sep=0pt] {};
    %----------------------------------

     \node (clf) [steps, left=2em of features] {Train classifier};
     ...
     \draw [arrow, myarrow] (ft) -- (aft);
     \draw [arrow, myarrow] (aft) -- (features);
     \draw [arrow, myarrow] (features) -- (clf);
...
Table~\ref{tab:results} shows the results of three FLIM-based solutions using the SVM and MLP classifiers; ``FLIM+SVM'', ``FLIM+MLP'', and ``FLIM-FT+MLP'' (i.e., FLIM with fine-tuning); as compared with the VGG-based solutions above. The first two rows indicate that FLIM  can provide a more effective feature extractor using a single convolutional layer than VGG-FE, which contains 13 convolutional layers pretrained on the ImageNet. The results of ``FLIM+MLP'' and ``FLIM-FT+MLP'' show that FLIM  may dismiss fine tuning by directly providing a more effective solution. By comparing ``FLIM+MLP'' with VGG, one can observe that FLIM reduces the number of required training images. Finally, the comparison between ``FLIM+MLP'' and VGG-FT demonstrates that FLIM can provide competitive results using a considerably more simplified architecture than  VGG-FT. The number of parameters of our feature extractor represents only $0.07\%$ of the number of parameter of VGG-FE.
