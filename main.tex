...
\begin{figure*}
  \begin{center}
    ...
    \begin{tikzpicture}[node distance=1.5cm]
     ...
     \node (imgs) [fit={($(img1.north west)+(-0.45,0.1)$) ($(img3.south east)+(0.45,-0.1)$)}, inner sep=0pt, draw=none, rounded corners, very thick, label = {[label distance = 0.1em]below:{Testing set}}] {};
     %----------------------------------

     \node (aft) [steps, right=2em of imgs] {Feature\\Extraction};
    ...
    %----------------------------------
     ...
     \node (sup) [steps, right=2em of features] {Classification};

     \node (clf) [steps, right=2em of sup, fill=none, draw=none] {Classifier};
...
\textcolor{red}{
The experiments have been designed to evaluate the following hypotheses about FLIM.
\begin{enumerate}
 \item[(H1)]  FLIM can produce competitive image feature extractors from user-drawn markers in a few images selected per class from a training set. 
...
\item[(H4)] User-drawn markers on a higher number of selected images from the training set can improve feature extraction.
\end{enumerate}
}
