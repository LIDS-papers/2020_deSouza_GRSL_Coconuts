...
\item[(H2)]  For a given network architecture, FLIM can provide better classification results than those obtained by training that network with backpropagation.
\item[(H3)] Its feature extractor remains competitive independently of the training set size. 
\item[(H4)] User-drawn markers on a higher number of selected images from the training set can improve image feature extraction.
\end{enumerate}
}
