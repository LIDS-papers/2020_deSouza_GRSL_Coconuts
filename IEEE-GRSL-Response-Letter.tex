...
For three splits only and 200 training images, it was not possible to claim that this difference is statistically significant. However, they are undoubtedly competitive, being FLIM+MLP a much simpler (lighter) network than VGG-FT. Even with a new experiment in which we increase the training set up to 1200 images, the difference increases in favor of FLIM+MLP, but they are still not statistically significant (except for one case with 1100 images). On the other hand, the f-score curve of FLIM+MLP was consistently above one of VGG-FT. We have improved the experimental section to clarify this and several other issues.  
\eans
...
\ans Thanks a lot for raising these questions. We noticed that it was not clear that markers are drawn on a few images selected from the training set (we used only four images). The user takes a couple of minutes to select images and draw markers in those images. This is much less than the time taken to examine and label 200 images. We also added an experiment with increasing training set sizes. VGG-FT could never beat FLIM+MLP. The reason is that the image features of FLIM are better than those of VGG-FE (the one extracted from the last convolutional layer of VGG-FT), and both use a similar MLP (the only difference is its input layer). So, higher is the number of training samples, MLP can improve the f-score of both, being the one of FLIM+MLP always above the one o    
